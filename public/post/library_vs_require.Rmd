...
It's considered better practice to use `library()` when we want to load packages outside of functions. This is because we would like to know if and where we have a bug as soon as possible. Calling a set of function which doesn't exists is certainly a bug. We'll use `require()` inside functions, as we can manipulate it's `TRUE`/`FALSE` output. For example, in the beginning of our script, we can write a function that will install all the necessary function if they aren't already installed [look here](). We could also skip parts of a script if the packages aren't already installed, etc. 
